...
var aHztCell = document.getElementsByClassName("roleContainer");
...
window.onload = function(){
    ...
    aHzt[index].getElementsByTagName("img")[0].style.height = wHeight - 160 + "px";

    for(var i = 0 ; i < aHztCell.length ; i ++){
        if(aHztCell[i].getElementsByClassName("img")[0]){
            var aHztCellImg = aHztCell[i].getElementsByClassName("img")[0];
            if(aHztCell[i].clientHeight < aHztCellImg.clientHeight + 100)
                aHztCell[i].style.height = aHztCellImg.clientHeight + 100 + "px";
        }
    }
